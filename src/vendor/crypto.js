...
function digest(algorithm, data1, length) {
  try {
...
const bufferSourceBytes = (data3)=>{
  let bytes;
  if (data3 instanceof Uint8Array) {
      bytes = data3;
  } else if (ArrayBuffer.isView(data3)) {
      bytes = new Uint8Array(data3.buffer, data3.byteOffset, data3.byteLength);
  } else if (data3 instanceof ArrayBuffer) {
      bytes = new Uint8Array(data3);
  }
  return bytes;
};
const stdCrypto = ((x)=>x
  ...
  subtle: {
      ...webCrypto.subtle,
      async digest (algorithm, data4) {
          const { name , length  } = normalizeAlgorithm(algorithm);
          const bytes = bufferSourceBytes(data4);
          if (webCryptoDigestAlgorithms.includes(name) && bytes) {
              return webCrypto.subtle.digest(algorithm, bytes);
          } else if (digestAlgorithms.includes(name)) {
              if (bytes) {
                  return stdCrypto.subtle.digestSync(algorithm, bytes);
              } else if (data4[Symbol.iterator]) {
                  return stdCrypto.subtle.digestSync(algorithm, data4);
              } else if (data4[Symbol.asyncIterator]) {
                  const context = new mod.DigestContext(name);
              ...
              return webCrypto.subtle.digest(algorithm, data4);
          } else {
              throw new TypeError(`unsupported digest algorithm: ${algorithm}`);
          }
      },
      digestSync (algorithm, data5) {
          algorithm = normalizeAlgorithm(algorithm);
          const bytes = bufferSourceBytes(data5);
          if (bytes) {
              return mod.digest(algorithm.name, bytes, algorithm.length).buffer;
          } else if (data5[Symbol.iterator]) {
              const context = new mod.DigestContext(algorithm.name);
              for (const chunk of data5){
                  const chunkBytes = bufferSourceBytes(chunk);
                  if (!chunkBytes) {
                      throw new TypeError("data contained chunk of the wrong type");
                  }
